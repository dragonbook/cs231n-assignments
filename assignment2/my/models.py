...
class SmallFilterWithFCBNNet(nn.Module):
    def __init__(self):
        super(SmallFilterWithFCBNNet, self).__init__()
...
class ExampleDifferentFiltersNet(nn.Module):
    def __init__(self, num_filters=32):
        super(ExampleDifferentFiltersNet, self).__init__()
        self.conv = nn.Conv2d(3, num_filters, kernel_size=7, stride=1)
        self.relu = nn.ReLU(inplace=True)
        self.bn = nn.BatchNorm2d(num_filters)
        self.max_pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.fc1 = nn.Linear(num_filters*13*13, 1024)
        self.fc2 = nn.Linear(1024, 10)
